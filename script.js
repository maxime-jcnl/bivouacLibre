      ...
      fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${query}`)
        ...
        .then((data) => {
          if (data.length > 0) {
            ...
            var coordinates = query.trim().split(/\s+/); // Utiliser trim() et une regex pour diviser la chaîne par des espaces multiples
      ...
      if (userInput.length > 2) {
        fetch(`https://photon.komoot.io/api/?q=${userInput}`)
          ...
          .then((data) => {
            ...
            // Vérifier si l'utilisateur a entré des coordonnées GPS
            var coordinates = userInput.trim().split(/\s+/); // Utiliser trim() et une regex pour diviser la chaîne par des espaces multiples
            if (coordinates.length === 2 && !isNaN(coordinates[0]) && !isNaN(coordinates[1])) {
              var suggestionItem = document.createElement("div");
