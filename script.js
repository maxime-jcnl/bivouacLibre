document.addEventListener("DOMContentLoaded", function () {
      ...
      fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${query}`)
        ...
        .then((data) => {
          if (data.length > 0) {
            ...
            var coordinates = extractCoordinates(query);
            if (coordinates) {
              executeSearch(coordinates.lat, coordinates.lon);
            } else {
      ...
      if (userInput.length > 2) {
        fetch(`https://photon.komoot.io/api/?q=${userInput}`)
          ...
          .then((data) => {
            ...
            // Vérifier si l'utilisateur a entré des coordonnées GPS
            var coordinates = extractCoordinates(userInput);
            if (coordinates) {
              var suggestionItem = document.createElement("div");
              suggestionItem.textContent = `Coordonnées GPS : ${coordinates.lat}, ${coordinates.lon}`;
              suggestionItem.classList.add("suggestion-item");
              suggestionItem.addEventListener("click", () => {
                document.getElementById("suggestions-container").style.display =
                  "none";
  ...
  // Fonction pour extraire les coordonnées à partir d'une chaîne de caractères
  function extractCoordinates(input) {
    var regex = /(?:Lat|latitude|Latitud|Lon|longitude|Longitud)\s*:\s*(-?\d+\.\d+)\s*,\s*(-?\d+\.\d+)/i;
